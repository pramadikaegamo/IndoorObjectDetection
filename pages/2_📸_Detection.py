...
def main():
    st.set_page_config(
        page_title="Object Detection", page_icon="🚀")

    st.title("Object Detection")

    # Create a sidebar for user inputs
    st.sidebar.title("Pilihan Konfigurasi")
    # capture_index = st.sidebar.number_input(
    #     "Nomor Index Video Capture", value=0, step=1)
    model_name = st.sidebar.text_input("Path Model", value="")

    # Create an object to detect objects
    detector = deteksi_objek(
        capture_index=0, model_name=model_name)
